...
fn do_tests_seq(test_cases: Vec<TestCaseSeq>) {
    for tt in test_cases {
        ...
        for (i, (expected, input)) in tt.results.iter().zip(tt.inputs).enumerate() {
            let msg: Message = fastlib::from_vec(&mut d, input).unwrap();
            assert_eq!(&msg, expected, "{} failed #{}", tt.name, i + 1);
        }
    }
}
